const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'CASGV 账号',
    langGeniusAccountTip: '您的 CASGV 账号和相关的用户数据。',
    editName: '编辑名字',
  ...
  members: {
    ...
    invitationSentTip: '邀请已发送，对方登录 CASGV 后即可访问你的团队数据。',
    invitationLink: '邀请链接',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: '体验',
      exhausted: '超出限额',
      desc: '托管 OpenAI 由 CASGV 提供的托管 OpenAI 服务，你可以使用 GPT-3.5 等模型，在体验额度消耗完毕前你需要设置其它模型供应商。',
      callTimes: '调用次数',
  ...
  apiBasedExtension: {
    title: 'API 扩展提供了一个集中式的 API 管理，在此统一添加 API 配置后，方便在 CASGV 上的各类应用中直接使用。',
  ...
  about: {
    changeLog: '更新日志',
    updateNow: '现在更新',
    nowAvailable: 'CASGV {{version}} 现已可用。',
