const translation = {
  ...
  completionMode: {
    title: '완성 모드 API',
    info: '문서, 요약, 번역 등 고품질 텍스트 생성을 위해 사용자 입력을 사용하는 완성 메시지 API를 사용합니다. 텍스트 생성은 CASGV Prompt Engineering에서 설정한 모델 매개변수와 프롬프트 템플릿에 의존합니다.',
  ...
  chatMode: {
    title: '채팅 모드 API',
    info: '질의 응답 형식을 사용하는 다목적 대화형 응용 프로그램에는 채팅 메시지 API를 호출하여 대화를 시작합니다. 반환된 conversation_id를 전달하여 계속된 대화를 유지합니다. 응답 매개변수 및 템플릿은 CASGV Prompt Eng의 설정에 의존합니다.',
