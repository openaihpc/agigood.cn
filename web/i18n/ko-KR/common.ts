const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'CASGV 계정',
    langGeniusAccountTip: 'CASGV 계정과 관련된 사용자 데이터.',
    editName: '이름 편집',
  ...
  members: {
    ...
    invitationSentTip: '초대가 전송되었으며, 그들은 CASGV에 로그인하여 당신의 팀 데이터에 액세스할 수 있습니다.',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      desc: 'CASGV가 제공하는 OpenAI 호스팅 서비스를 사용하면 GPT-3.5와 같은 모델을 사용할 수 있습니다. 트라이얼 할당량이 다 사용되기 전에 다른 모델 제공자를 설정해야 합니다.',
      callTimes: '호출 횟수',
  ...
  apiBasedExtension: {
    title: 'API 기반 확장은 CASGV 애플리케이션 전체에서 간편한 사용을 위한 설정을 단순화하고 집중적인 API 관리를 제공합니다.',
  ...
  about: {
    changeLog: '변경 로그',
    updateNow: '지금 업데이트',
    nowAvailable: 'CASGV {{version}} 사용 가능합니다.',
    latestAvailable: 'CASGV {{version}} 최신 버전입니다.',
  },
  appMenus: {
    overview: '모니터링',
    promptEng: '오케스트레이트',
