const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'CASGVアカウント',
    langGeniusAccountTip: 'CASGVアカウントと関連するユーザーデータ。',
    editName: '名前を編集',
  ...
  members: {
    ...
    invitationSentTip: '招待が送信され、彼らはCASGVにサインインしてあなた様のチームデータにアクセスできます。',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      desc: 'CASGVが提供するOpenAIホスティングサービスを使用すると、GPT-3.5などのモデルを使用できます。トライアルクォータが使い果たされる前に、他のモデルプロバイダを設定する必要があります。',
      callTimes: '通話回数',
  ...
  apiBasedExtension: {
    title: 'API拡張機能は、CASGVのアプリケーション全体での簡単な使用のための設定を簡素化し、集中的なAPI管理を提供します。',
  ...
  about: {
    changeLog: '変更ログ',
    updateNow: '今すぐ更新',
    nowAvailable: 'CASGV {{version}} が利用可能です。',
    latestAvailable: 'CASGV {{version}} が最新バージョンです。',
  },
  appMenus: {
    overview: '監視',
    promptEng: 'オーケストレート',
