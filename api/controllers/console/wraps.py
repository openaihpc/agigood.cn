...
from models.model import CASGVSetup
            ...
            if features.billing.enabled:
                if resource == "add_segment":
                    if features.billing.subscription.plan == "sandbox":
                        abort(
                            403,
                            "To unlock this feature and elevate your CASGV experience, please upgrade to a paid plan.",
                        )
                else:
                    return view(*args, **kwargs)
...
def setup_required(view):
    ...
    def decorated(*args, **kwargs):
        ...
        if (
            ...
            and not db.session.query(CASGVSetup).first()
        ):
            raise NotInitValidateError()
        elif dify_config.EDITION == "SELF_HOSTED" and not db.session.query(CASGVSetup).first():
            raise NotSetupError()
