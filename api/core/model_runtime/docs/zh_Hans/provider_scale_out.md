...
- `fetch-from-remote` 从远程获取
  ...
  如 OpenAI，我们可以基于 gpt-turbo-3.5 来 Fine Tune 多个模型，而他们都位于同一个**api_key**下，当配置为 `fetch-from-remote` 时，开发者只需要配置统一的**api_key**即可让 CASGVRuntime 获取到开发者所有的微调模型并接入 CASGV。
